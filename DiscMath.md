...
So for $\Z_p$ ($0..p-1$), each number has a modular additive inverse, written by abuse of notation as $n^{-1}\pmod{p}$:
$$\forall n\in\Z_p\ .\ n^{p-2} \equiv n^{-1}\pmod{p}\tag{2.3}$$
...
RTP:&ensp; $\forall d\in\Z^+.\ (d\in\text{CD}(n + km, m)\iff d\in\text{CD}(n, m))$.  
&emsp;i.e.&nbsp; $\forall d\in\Z^+.\ (d\mid n+km)\ \land\ d\mid m\iff d\mid n\ \land\ d\mid m$.  
Let $d\in\Z^+$.  
...
RTP:&ensp; $id = n+km \iff jd=n$&emsp; for some $i,j\in\Z$.  
